...
from torch_geometric.nn import global_mean_pool, global_max_pool, GATConv, TopKPooling
...
class GATN(nn.Module):
    def __init__(self,
        ...
        self.linear1 = Linear(hidden_dim * 6, 256)
        self.linear2 = Linear(256, 1024)
        self.linear3 = Linear(1024, output_dim)
        
    def forward(self, mol_graph: Data) -> Tensor:
        x = mol_graph.x
...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d, 
        ...
        self.tanh = tanh
        self.epsilon = 1e-8 # normalizing in coor2radial
        edge_coords_nf = 1
        # node_attr_nf = 11
        ...
        self.node_mlp = nn.Sequential(
            nn.Linear(hidden_nf + input_nf, hidden_nf), # INPUT dim에 node_attr_nf 제거
            act_fn,
    ...
    def forward(self, 
        ...
        # h[row], h[col] -> src atom's feat의 리스트, tar atom's feat의 리스트
        edge_feat = self.edge_model(h[row], h[col], radial, edge_attr) # edge_feat = msg btw nodes
...
class EGNN(nn.Module):
    ...
    def forward(self, 
                ...
                edges: List[LongTensor], # edge_idx
                edge_attr: Tensor,
                node_attr: Tensor, # Not to use?
                batch: Tensor,) -> Tuple[Tensor, Tensor]:
        h = self.embedding_in(h)
        for i in range(0, self.n_layers):
            h, x, _ = self._modules["gcl_%d"%i](h, edges, x, edge_attr=edge_attr, node_attr=None) # node_attr 제거?
