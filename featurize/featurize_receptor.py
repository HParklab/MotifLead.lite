...
def featurize_target_properties(pdb, inputpath, outf, store_npz=True, extra={}, verbose=False): # -> save *.prop.npz
    ...
    # calculate sec & bb dihedrals, assume chain order is not shuffled...
    chains = np.unique([rc.split('.')[0] for rc in reschains])
    SS3,phi,psi = [],[],[]
    for chain in chains:
        xyz_chain = [xyz[rc] for rc in reschains if rc.split('.')[0] == chain]
        reslist = [int(rc.split('.')[1]) for rc in reschains if rc.split('.')[0] == chain]
        SS3_chain, phi_chain, psi_chain = get_chain_SS3(xyz_chain, reslist)
        ...
        for i,res in enumerate(reslist):
            reschain = chain+'.%d'%res
            SS3 += [SS3_chain[i] for _ in range(nresatm[reschain])]
            phi += [phi_chain[i] for _ in range(nresatm[reschain])]
            psi += [psi_chain[i] for _ in range(nresatm[reschain])]
            
    SS3 = one_hot_encode(SS3)
    phi = np.array(phi)
    psi = np.array(psi)

    if verbose:
        for a,s,arc in zip(aas_rec, SS3, atmres_rec):
            print(arc,a,s)

    bb_dihe = np.array([np.cos(phi), np.sin(phi), np.cos(psi), np.sin(psi)]).transpose(1,0)
    elems = [a[0] for a in atypes_rec] # works for protein.... but can be dangerous for general molecules
...
def main(pdb,
    ...
    status = featurize_target_properties(pdb, inputpath,
                                         outf,
                                         verbose=verbose)
    if not status:
        print("skip ", pdb)
